...
/**
 * The Page Renderer (for DOCX format)
 * 
 * @package DokuWiki\Renderer\Page
 */
class renderer_plugin_odt_pagedocx extends renderer_plugin_odt_page {
    ...
    function document_start() {
        ...
        // Create HTTP headers
        $output_filename = str_replace(':','-',$ID).'.'.$this->getConf('doc file extension');
        
        $headers = array(
            'Content-Type' => 'application/'.$this->getConf('doc file extension'),
        ...
        p_set_metadata($ID,array('format' => array('odt_pagedocx' => $headers) ));
    }
    ...
    protected function convert () {
        ...
        if ($format == 'docx') {
            ...
            $docx_file = $temp_dir.'/convert.'.$this->getConf('doc file extension');

            // Prepare command line
            $command = $this->getConf('doc convert command');
            ...
            if ($result) {
                ...
                $this->doc = io_readFile($docx_file, false);
            }
